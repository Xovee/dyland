...
class CNF(nn.Module):
    ...
    def forward(self, z, std, logpz=None, integration_times=None, reverse=False):
        if logpz is None:
        ...
        # https://github.com/rtqichen/torchdiffeq#basic-usage   (z, std, _logpz) as input y0, and use the related output
        # to calculate the loss, and therefore automatically compute _logpz without numerical form
        if self.training:
            state_t = odeint(
                self.odefunc,
                (z, std, _logpz),
